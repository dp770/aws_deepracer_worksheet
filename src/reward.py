...
FOLLOWING_CENTRAL_LINE_RATIO = 0.02  # number in range of [0, 1]. Zero forces to follow smothed line, 1 - central line
...
TOTAL_PENALTY_ON_HIGH_STEERING = 0.30  # maximum penalty in percentage of total reward on high streering
REWARD_WEIGHT_ON_TRACK = 7.00
REWARD_WEIGHT_DIR_STEER = 1.50
REWARD_WEIGHT_MAX_SPEED = 1.00
REWARD_WEIGHT_PROG_STEP = 0.50
REWARD_WEIGHT_MIN_STEER = 0.00
...
def calc_distance_from_line(curr_point, prev_point, next_point):
    distance_c_to_pp = calc_distance(curr_point, prev_point)  # b
    distance_c_to_np = calc_distance(curr_point, next_point)  # a
    distance_pp_to_np = calc_distance(prev_point, next_point)  # c
    # cos A = (b^2 + c^2 - a^2) / 2bc
    angle_pp = math.acos((distance_c_to_pp * distance_c_to_pp + distance_pp_to_np * distance_pp_to_np
                          - distance_c_to_np * distance_c_to_np) / (2 * distance_c_to_pp * distance_pp_to_np))
    # b / sin(Pi/2) = d / sin(A)
    return distance_c_to_pp * math.sin(angle_pp)
...
def reward_function(params):
    ...
    # locals
    off_track_penalty = min(steps - was_off_track_at_step, MAX_STEPS_TO_DECAY_PENALTY) / (
                1.0 * MAX_STEPS_TO_DECAY_PENALTY)
    reward_on_track = 0.0
    reward_max_speed = 0.0
    reward_min_steering = 0.0
    reward_dir_steering = 0.0
    reward_prog_step = 0.0
    ...
    direction_diff_ratio = 0.20 * min((calc_direction_diff(steering, heading, track_direction_1) / MAX_DIRECTION_DIFF),
                                      1.00)
    direction_diff_ratio += 0.30 * min((calc_direction_diff(steering, heading, track_direction_2) / MAX_DIRECTION_DIFF),
                                       1.00)
    direction_diff_ratio += 0.50 * min((calc_direction_diff(steering, heading, track_direction_3) / MAX_DIRECTION_DIFF),
                                       1.00)
    reward_dir_steering = REWARD_WEIGHT_DIR_STEER * (1.0 - pow(direction_diff_ratio, DIR_STEERING_SENSITIVITY_EXP))
    ...
    # Calculate dislocation from smoothed line
    ...
    if dislocation <= free_zone:
        reward_on_track = REWARD_WEIGHT_ON_TRACK
    elif dislocation <= safe_zone:
        reward_on_track = REWARD_WEIGHT_ON_TRACK * (1.0 - pow(dislocation / safe_zone, CNT_DISTANCE_SENSITIVITY_EXP))
    ...
    reward_total -= reward_total * steering_ratio * steering_ratio * TOTAL_PENALTY_ON_HIGH_STEERING

    print("rewards:" + (20 * "{:.4f}," + "{:.4f}").format(
        off_track_penalty, reward_on_track, reward_max_speed, reward_min_steering, reward_dir_steering,
        reward_prog_step,
    ...
    return float(0.0001 + off_track_penalty * reward_total)
